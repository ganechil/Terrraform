...
data "azuread_service_principal" "azuredatabricks" { display_name = "AzureDatabricks" }

locals { stgaccname = "${var.stgaccname}${var.company}${var.env}01" }
...
resource "databricks_cluster" "dbcluster01" {
  ...
  #instance_pool_id = databricks_instance_pool.instancepool.id // instadnce pool id reference
